...
{
    var requests = new ReactiveArray(Requests.find({"owner": Meteor.userId()}, {sort: {created_on: -1}}).fetch());
    for(r in requests){
      if(requests[r].requestor){
...
Template.dashboard.onRendered(function(){
  this.autorun(function(){
    var tcount = tasksCursor().count();
    var pcount = projCursor().count();
    var rcount = requestsCursor().depend();
